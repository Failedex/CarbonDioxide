...
class Timer(dbus.service.Object): 
    ...
    def Toggle(self): 
        if self.timer: 
            self.timer = None            
            self.UpdateEww()
            # os.popen(f"notify-send -a Timer 'Timer' 'stopped'")
        else: 
            self.timer = datetime.now().timestamp() + 60*self.minutes

            # hours = self.minutes // 60
            # if hours > 0:
            #     os.popen(f"notify-send -a Timer 'Timer' 'started for {hours} hours and {self.minutes} minutes'")
            # else:
            #     os.popen(f"notify-send -a Timer 'Timer' 'started for {self.minutes} minutes'")
    ...
    def Loop(self): 
        while self.timer:
            if self.timer - datetime.now().timestamp() < 0:
                self.timer = None
                self.UpdateEww()
                os.popen(f"notify-send -a Timer 'Timer' 'time is up'")
                break
    ...
    def UpdateEww(self): 
        ...
        if self.timer: 
            ...
            info["duration"] = sec*100/(60*self.minutes)
            info["running"] = True
        else:
